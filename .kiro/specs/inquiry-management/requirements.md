...
2. WHEN 작성자가 아닌 회원이 문의 상세를 조회하면, THE System SHALL 조회수를 정확히 1 증가시킨다. ✅
3. IF 작성자 본인이 자신의 문의 상세를 조회하면, THEN THE System SHALL 조회수를 증가시키지 않는다(셀프 카운트 방지). ✅
4. THE System SHALL 화면에 표시되는 조회수가 저장된 최신 조회수와 일치하도록 한다. ✅
5. THE System SHALL 작성자 이름을 회원 테이블과 조인하여 조회한다. ✅
6. IF 존재하지 않는 문의 ID로 접근하면, THEN THE System SHALL HTTP 404를 반환한다. ✅
...
| IR-04 | 작성자가 아닌 회원의 상세 조회마다 조회수가 1 증가하고, 작성자 본인 조회는 증가하지 않는다 | ✅ |
...
| Q2 | 조회수 중복 증가 방지 | 작성자 본인 조회는 증가하지 않음. 그 외 회원은 접근마다 증가(추가 중복 방지는 미구현) |
| Q3 | 관리자 답변·처리 상태 | 미구현 |
