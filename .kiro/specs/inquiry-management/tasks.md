...
```json
{
  "waves": [
    ...
    { "id": 4, "tasks": ["8.1", "8.2", "8.3", "8.4", "8.5", "9.1", "9.2", "9.3", "9.4", "9.5", "9.6"] }
  ]
}
```

- wave 0(도메인·스키마)이 문의 작성·조회의 선행이다.
- wave 1에서 작성(2)·목록(3)·상세(4)·접근 제어(5)를 함께 다룬다.
- wave 2(테스트) → wave 3(문서) 순으로 검증·정리한다.
- wave 4(8·9)는 구현과 독립적인 PI/PO 의사결정·백로그로 마지막에 둔다.
